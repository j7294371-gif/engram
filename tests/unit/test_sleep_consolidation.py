...
@pytest.fixture
def populated_backend(backend: InMemoryBackend) -> InMemoryBackend:
...
class TestSleepConsolidation:
    def test_empty_run_no_errors(self, backend: InMemoryBackend):
        consolidator = SleepConsolidation(backend=backend)
        report = consolidator.run()
        assert isinstance(report, ConsolidationReport)
        assert report.promotions == 0

    def test_promotes_high_importance_working(self, populated_backend: InMemoryBackend):
        ...
        report = consolidator.run(working_items=working_items)
        assert report.promotions >= 1

        # Verify wm_1 was promoted to episodic
        wm1_promoted = False
        all_items = populated_backend.list()
    ...
    def test_archives_forgotten_memories(self, populated_backend: InMemoryBackend):
        consolidator = SleepConsolidation(
            backend=populated_backend,
        ...
        old_item = populated_backend.get("old_1")
        assert old_item is not None
        assert old_item.consolidation_stage == ConsolidationStage.ARCHIVED

    def test_extracts_semantic_abstractions(self, populated_backend: InMemoryBackend):
        ...
        sem_items = populated_backend.list(memory_types=[MemoryType.SEMANTIC])
    ...
    def test_strengthens_frequent_memories(self, populated_backend: InMemoryBackend):
        # Give one item low strength but high access count
        ep1 = populated_backend.get("ep_1")
        assert ep1 is not None
        ep1.strength = 0.5
        ep1.access_count = 10
        populated_backend.update(ep1)
        ...
        refreshed = populated_backend.get("ep_1")
        assert refreshed is not None
        assert refreshed.strength > 0.5

    def test_report_contains_metrics(self, backend: InMemoryBackend):
        consolidator = SleepConsolidation(backend=backend)
        report = consolidator.run()
